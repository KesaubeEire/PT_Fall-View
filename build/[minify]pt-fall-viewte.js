...
`,A),g(a()),w(x),Ve.success("捕获到 /search !"),l=A.data,c?c.updateList(l,u):c=br(gl,{target:f,props:{infoList:l}});});}else Ve.error(`找不到指定节点
...
这可能导致显示错误
请确认您不在一般的瀑布流视图下
比如您在逛论坛或者在发种之类的被遮挡了再点`)&&(L.style.display="none");}),b.querySelector("#_shield")||b.appendChild(L);const A=document.createElement("div");A.id="_fallHolder",b.querySelector("#_fallHolder")||(b.appendChild(A),A.appendChild(f));}let m="",k="",$="";function g(x){f.style.display=x?"block":"none",Le("#_fallHolder",b=>{b.style.display=x?"block":"none";}),Le("#_shield",b=>{b.style.display=x?"block":"none";}),Le(Te.TL_Selector+".flex",b=>{k||(k=getComputedStyle(b).getPropertyValue("max-width"),$=getComputedStyle(b).getPropertyValue("padding-left")),b.style.maxWidth=x?"none":k,b.style.paddingLeft=x?"80px":$,b.style.paddingRight=x?"80px":$;}),Le(Te.TL_Selector+" .mx-auto",b=>{m||(m=b.style.margin),b.style.margin=x?0:m;});}window.changeFallView=g;const C=history.pushState;function O(){history.pushState=function(x,b,L){console.log(`%c ====> URL跳转劫持: %c${L}`,"color: cyan","color: white"),L.includes("/browse")||L=="/waterfall"?console.log(`--->属于 browse 范围: ${L}`):(console.warn(`[FALL]: --->不属于 browse 范围: ${L}`),g(false)),C.apply(history,arguments);};}function N(x){return getComputedStyle(document.documentElement).getPropertyValue(x).trim()}function E(){ht(Ne,I(i).t1=Ut(N("--bg-1")),I(i)),ht(Ne,I(i).t2=Ut(N("--bg-2")),I(i)),ht(Ne,I(i).t3=Ut(N("--bg-3")),I(i));}ye(),le(),n();}function Ll(t){if(location.hostname.includes("m-team"))return Te}var Ml=H('<div class="switch-container svelte-18ntgfp"><div class="switch-background svelte-18ntgfp"><div class="switch-slider svelte-18ntgfp"></div></div></div>');function Mt(t,e){let r=ft(e,"checked",12,false);function n(){r(!r());}var a=Ml(),i=h(a),o=h(i);j(()=>cr(o,"checked",r())),W("click",a,n),M(t,a);}var Nl=Ze(`<svg viewBox="0 0 32 32" width="20" height="20" xmlns="http://www.w3.org/2000/svg" stroke="currentColor"><defs><style>.cls-1 {
        fill: none;
